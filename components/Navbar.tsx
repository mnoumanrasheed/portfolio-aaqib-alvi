...
const focus = "focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-4";
const cta = "group inline-flex min-h-11 items-center gap-4 border-b border-gold/60 py-2 text-sm font-medium text-accent transition-colors duration-300 hover:border-accent-dark hover:text-accent-dark";
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 font-sans tracking-normal">
      <div aria-hidden="true" className={cn(
        ...
        scrolled || open ? "border-[rgba(20,35,45,0.08)] bg-[rgba(246,245,241,0.90)] backdrop-blur-xl" : "border-transparent bg-transparent"
      )} />
      <div className="relative mx-auto flex h-20 max-w-7xl items-center justify-between gap-8 px-5 sm:px-8 lg:px-10">
        <Link href="/" aria-label="Aaqib Alvi homepage" className={cn("group flex shrink-0 items-center gap-3", focus)}>
          <div className="relative h-10 w-10 overflow-hidden rounded-full border border-ink/10 transition-colors duration-300 group-hover:border-gold/60">
            <Image src="/logo.png" alt="" fill sizes="40px" className="object-cover" priority />
          </div>
          <div className="flex flex-col gap-1">
            <span className="font-display text-[24px] font-normal leading-none text-ink">AAQIB ALVI</span>
        ...
        <nav aria-label="Main navigation" className="hidden items-center gap-8 lg:flex xl:gap-12">
          {NAV_ITEMS.map((item) => (
            <Link key={item.href} href={item.href} aria-current={active(item.href) ? "page" : undefined}
              className={cn("group relative flex min-h-11 items-center whitespace-nowrap text-[13px] font-medium transition-colors duration-300 hover:text-accent", focus, active(item.href) ? "text-accent" : "text-[#53616A]")}>
              {item.label}
              <span aria-hidden="true" className={cn("absolute inset-x-0 bottom-0 h-px origin-left bg-accent transition-transform duration-500 motion-reduce:transition-none", active(item.href) ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100 group-focus-visible:scale-x-100")} />
            </Link>
          ))}
        </nav>
        ...
        <button ref={toggle} type="button" aria-label={open ? "Close navigation" : "Open navigation"} aria-expanded={open} aria-controls="mobile-navigation"
          onClick={() => setOpen((value) => !value)} className={cn("flex h-11 w-11 shrink-0 items-center justify-center text-ink hover:text-accent lg:hidden", focus)}>
      ...
      <AnimatePresence>
        {open && (
          <motion.div id="mobile-navigation" ref={panel}
            ...
            className="fixed inset-x-0 bottom-0 top-20 overflow-y-auto border-t border-ink/10 bg-page px-6 py-8 sm:px-10 lg:hidden">
            <nav aria-label="Mobile navigation" className="mx-auto flex max-w-xl flex-col">
              {NAV_ITEMS.map((item) => (
                <Link key={item.href} href={item.href} onClick={() => setOpen(false)} aria-current={active(item.href) ? "page" : undefined}
                  className={cn("group flex min-h-20 items-center justify-between gap-5 border-b border-ink/10 py-5 font-display text-[28px] leading-tight transition-colors duration-300 hover:text-accent", focus, active(item.href) ? "text-accent" : "text-ink")}>
                  <span>{item.label}</span>
                  <ArrowUpRight aria-hidden="true" className="h-4 w-4 shrink-0 text-accent" />
                </Link>
              ))}
