...
const controlStyle = "inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-sm border border-border bg-white text-muted transition-colors duration-300 hover:border-accent/35 hover:bg-surface-teal hover:text-accent-dark focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2 disabled:opacity-40 motion-reduce:transition-none";
...
export default function HeroOrbit() {
  ...
  return (
    <div ref={root} role="region" aria-label="Leadership expertise network" onFocusCapture={() => setFocused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setFocused(false); }} className="relative flex w-full min-w-0 max-w-[500px] flex-col items-center font-sans tracking-normal">
      ...
      >
        <div aria-hidden="true" className="absolute inset-[16%] rounded-full bg-accent/10 blur-3xl" />
        <div aria-hidden="true" className="absolute inset-[25%] rounded-full bg-gold/10 blur-2xl" />
          ...
          <svg role="group" aria-label="Six expertise nodes" viewBox="0 0 520 520" className="block h-full w-full overflow-hidden">
            <defs>
              <radialGradient id="aaCoreGlow" cx="50%" cy="45%" r="62%">
                ...
                <stop offset="58%" stopColor="#78A5A3" stopOpacity="0.12" />
                <stop offset="100%" stopColor="#F6F5F1" stopOpacity="0" />
              </radialGradient>
              <linearGradient id="activeConnection" x1="260" y1="260" x2={activePoint.x} y2={activePoint.y} gradientUnits="userSpaceOnUse">
                <stop offset="0%" stopColor="#14232D" stopOpacity="0.12" />
                <stop offset="62%" stopColor="#78A5A3" stopOpacity="0.48" />
                <stop offset="100%" stopColor="#A27C42" stopOpacity="0.66" />
              </linearGradient>
            </defs>
              ...
              {CAPABILITIES.map((item, index) => {
                ...
                return (
                  <g key={item.id} role="button" tabIndex={0} aria-label={`${item.id}: ${item.shortLabel}`} aria-pressed={selected} onClick={() => select(index)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); select(index); } }} className="group cursor-pointer outline-none">
                    ...
                    {selected ? (
                      <circle cx={point.x} cy={point.y} r="21" fill="#A27C42" opacity="0.13" />
                    ) : null}
                    <circle cx={point.x} cy={point.y} r="17" fill="none" stroke="#A27C42" strokeWidth="2" className="opacity-0 group-focus-visible:opacity-100" />
                    <circle cx={point.x} cy={point.y} r={selected ? 11 : 7} fill="#FFFFFF" stroke={selected ? "#A27C42" : "#276B6D"} strokeOpacity={selected ? 0.95 : 0.72} strokeWidth={selected ? 1.4 : 1} />
                    <circle cx={point.x} cy={point.y} r={selected ? 3.5 : 2.15} fill={selected ? "#A27C42" : "#276B6D"} />
                  </g>
                );
              })}
            </motion.g>
            <circle cx="260" cy="260" r="72" fill="#FFFFFF" fillOpacity="0.92" stroke="#78A5A3" strokeOpacity="0.18" strokeWidth="14" />
            <circle cx="260" cy="260" r="66" fill="#FFFFFF" stroke="#14232D" strokeOpacity="0.1" strokeWidth="1.15" />
            <circle cx="260" cy="260" r="55" fill="none" stroke="#A27C42" strokeOpacity="0.22" strokeWidth="0.8" />
            <path d="M 206 226 A 68 68 0 0 1 292 203" fill="none" stroke="#14232D" strokeOpacity="0.18" strokeWidth="1" />
            <text x="260" y="257" textAnchor="middle" dominantBaseline="middle" fontFamily="Instrument Serif, Georgia, serif" fontSize="64" fontWeight="400" letterSpacing="0" fill="#14232D">AA</text>
            <text x="260" y="304" textAnchor="middle" dominantBaseline="middle" fontFamily="Geist, Arial, sans-serif" fontSize="10" fontWeight="500" letterSpacing="1.8" fill="#276B6D" opacity="0.78">LEADERSHIP</text>
          </svg>
        </motion.div>
      </div>

      <div className="relative w-full max-w-[452px] overflow-hidden rounded-lg border border-ink/10 bg-white/90 px-5 py-5 shadow-glass backdrop-blur-[2px] sm:px-6">
        ...
        <div aria-hidden="true" className="absolute inset-0 bg-[linear-gradient(135deg,rgba(39,107,109,0.055),transparent_42%,rgba(162,124,66,0.045))]" />
        <div className="relative flex items-center justify-between gap-4 text-[11px] font-medium leading-5">
          <span className="tabular-nums text-muted"><span className="text-gold">{CAPABILITIES[active].id}</span> / 06</span>
          <span className="truncate uppercase text-accent">{CAPABILITIES[active].shortLabel}</span>
        </div>
        ...
        <div aria-live={canMove && !focused ? "off" : "polite"} aria-atomic="true" className="relative mt-4 grid">
          {CAPABILITIES.map((item, index) => (
            <motion.div key={item.id} aria-hidden={index !== active} initial={false} animate={{ opacity: index === active ? 1 : 0, y: reducedMotion || index === active ? 0 : 8 }} transition={reducedMotion ? { duration: 0 } : informationTransition} className={`col-start-1 row-start-1 ${index === active ? "" : "pointer-events-none"}`}>
              <h2 className="max-w-[390px] text-[1.35rem] font-medium leading-[1.22] text-ink sm:text-[1.45rem]">{item.title}</h2>
              <p className="mt-3 max-w-[410px] text-sm leading-6 text-text">{item.description}</p>
            </motion.div>
          ))}
        </div>
        <div role="group" aria-label="Choose expertise" className="relative mt-5 grid grid-cols-6 gap-2">
          {CAPABILITIES.map((item, index) => (
            <button key={item.id} type="button" onClick={() => select(index)} aria-label={`Show ${item.shortLabel}`} aria-pressed={index === active} title={item.shortLabel} className="group flex min-h-11 min-w-0 flex-col justify-center gap-2 rounded-sm focus-visible:outline focus-visible:outline-2 focus-visible:outline-accent focus-visible:outline-offset-2">
              <span className={`text-[10px] font-medium tabular-nums transition-colors duration-300 motion-reduce:transition-none ${index === active ? "text-gold" : "text-muted group-hover:text-accent"}`}>{item.id}</span>
              <span className={`h-px w-full transition-all duration-500 motion-reduce:transition-none ${index === active ? "bg-gold" : "bg-border group-hover:bg-accent/35"}`} />
            </button>
          ))}
        </div>
        <div role="group" aria-label="Explore capabilities" className="relative mt-1 flex items-center justify-between border-t border-border pt-3">
